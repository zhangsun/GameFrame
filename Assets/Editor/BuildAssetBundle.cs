...
{
    //assetbundle资源
    static string assetFilePath = "/assetbundles";///Resources";
   // static string buildCfgFilePath = Application.dataPath + "/Editor/Build/build.txt";
    ...
    //打包环境设置
    static BuildAssetBundleOptions options = BuildAssetBundleOptions.None;
    /// <summary>
    /// 打包平台
    /// </summary>
    ...
    static Dictionary<string, int> allAssetsSize = new Dictionary<string, int>();
    ...
    private static void AddAssetSizeToAssetInfoFile()
    {
        ...
        for (int i = 0; i < xnl.Count; i++)
        {
            ...
            //根据路径获取size大小
            string unitName = "Assets/" + assetName;
            int size = GetAssetSize(unitName.ToLower());
            xe.SetAttribute("bundlesize", size.ToString());
        }

        xmlDoc.Save(ResourceCommon.assetbundleFilePath + "AssetInfo.bytes");

        Debug.Log("AddAssetSizeToAssetInfoFile success!!!");
    }
    ...
    public static void DumpVersionFile()
    {
        ...
        foreach (string element in allFiles)
        {
            ...
            allAssetsSize.Add(assetName.ToLower(), size);
        }
        ...
        //拷贝到对应的Resource目录
        string resourceVersionPath = Application.dataPath + "/res/" + "Version.bytes";
    ...
    private static void BuildResource()
    {
        ...
        for (int level = 1; level <= allLevelAssets.Count; level++)
        {
            ...
            foreach (KeyValuePair<string, AssetUnit> pair in levelAssets)
            {
                ...
                if (null == asset)
                    Debug.LogError("load " + pair.Value.mPath + " failed!!!" + "BuildResource");


                //设置ab的 name 和 variant
                AssetImporter importer = AssetImporter.GetAtPath(pair.Value.mPath);
                // importer.assetBundleName =BuildCommon.getPath(pair.Value.mPath).Replace("Assets/", string.Empty)+"/" + asset.name + ".unity3d";
                importer.assetBundleName = pair.Value.mPath.Replace("Assets/", string.Empty);
                importer.assetBundleVariant = ResourceCommon.assetbundleFileSuffix;//= "variant";

            }
        }

        //生成打包保存路径
        string savePath = Application.dataPath + assetFilePath;
        Debug.Log("savePath :" + savePath);
        //BuildCommon.CheckFolder(BuildCommon.getPath(savePath));
        if (!Directory.Exists(savePath))
            Directory.CreateDirectory(savePath);
        BuildPipeline.BuildAssetBundles(savePath, options, buildPlatform);
    }
    ...
    /// 获取需要打包的所有资源
    /// </summary>
    private static void GetBuildResources()
    {
        mResources.Clear();

        //Resources资源路径
        string resourcePath = Application.dataPath + "/res/";
        ...
        foreach (var file in files)
        {
            ...
            if (realFile == "Assets/res/Version.bytes")
                continue;

            string folder = BuildCommon.getPath(realFile);
            if (folder == "Assets/res/scenes")
                continue;
    ...
    /// 创建Asset资源信息
    /// </summary>
    private static void DumpAssetInfoFile()
    {
    ...
    {
        if (allAssetsSize.Count == 0)
            return 0;
        if (allAssetsSize.ContainsKey(name))
            return allAssetsSize[name];
        return 0;
    }
    //对AssetUnit进行排序
